...
$(function () {
  ...
  $('.blog-items__button-link').on('click', function (e) {
    e.preventDefault();
    $('body, html').animate({
      scrollTop: top
    }, 0);
...
var Calculator = /*#__PURE__*/function () {
  function Calculator(form) {
    ...
    this.catet2 = null;
    this.gip = null;
    this.formuls = ['c = <span>a<sup>2</sup> + b<sup>2</sup></span>', 'a = <span>c<sup>2</sup> - b<sup>2</sup></span>', 's = (A × B)/2 '];
    this.render();
  }
  ...
  _createClass(Calculator, [{
    ...
    value: function clickHandler() {
      ...
      var select = this.form.querySelector('[data-type="select"]');
      document.addEventListener('click', function (event) {
        ...
        if (type === 'counter') {
          ...
          _this3.clearValueError(_this3.form);
        } else {
      ...
      select.addEventListener('click', function (event) {
        ...
        if (id === 'cat') {
          ...
          _this3.formula = _this3.formuls[1];
        ...
        if (id === 'gip') {
          ...
          _this3.value = 'gip';
          _this3.formula = _this3.formuls[0];
        ...
        if (id === 's') {
          ...
          _this3.value = 's';
          _this3.formula = _this3.formuls[2];
    ...
    value: function input() {
      ...
      this.form.addEventListener('input', function (event) {
        var _event$target$dataset2 = event.target.dataset,
            type = _event$target$dataset2.type,
            info = _event$target$dataset2.info;

        var $el = _this4.form.querySelector("[data-type=\"".concat(type, "\"]"));

        var userValue = $el.value.trim().toLowerCase();
        var keyWords = ['катет', 'гипотенуза', 'площадь'];
        var inputValues = ['value-1', 'value-2', 'value-3'];
        var formuls = ['c = <span>a<sup>2</sup> + b<sup>2</sup></span>', 'a = <span>c<sup>2</sup> - b<sup>2</sup></span>', 's = (A × B)/2 '];
        ...
        if (type === 'counter') {
          if (keyWords.includes(userValue)) {
            ...
            switch (true) {
              case userValue === keyWords[0]:
                _this4.value = 'catet';
                _this4.inputType = 'Катет';
                _this4.formula = _this4.formuls[1];
              ...
              case userValue === keyWords[1]:
                _this4.inputType = 'Гипотенуза';
                _this4.value = 'gip';
                _this4.formula = _this4.formuls[0];
              ...
              case userValue === keyWords[2]:
                _this4.inputType = 'Площадь';
                _this4.value = 's';
                _this4.formula = _this4.formuls[2];
          ...
          if (!Number.isNaN(countValue)) {
            ...
            switch (true) {
              case info === 'catet':
                _this4.catet = $el.value;
                break;

              case info === 'catet2':
                _this4.catet2 = $el.value;
                break;
    ...
    value: function submit(err) {
      ...
      $button.addEventListener('click', function (event) {
        if (err) {
          $ansver.textContent = _this5.valueError('введены некорректные значения');
        } else {
          event.preventDefault();

          switch (true) {
            case _this5.value === 'catet':
              var calc = calcCatet(_this5.gip, _this5.catet);
              $ansver.textContent = Number.isNaN(calc) ? _this5.valueError('гипотенуза не может быть меньше катета') : _this5.clearValueError(calc);
              break;

            case _this5.value === 'gip':
              $ansver.textContent = calcGip(_this5.catet, _this5.catet2);
              break;

            case _this5.value === 's':
              var calculate = calcS(_this5.catet, _this5.catet2);
...
var calcS = function calcS(catet, catet2) {
  return catet * catet2 / 2;
};
  ...
  _createClass(Gallery, [{
    ...
    value: function overlayHandler() {
      ...
      this.$overlay.addEventListener('click', function (event) {
        selectOpener(_this6.$img).close();
        selectOpener(_this6.$overlay).close();
        document.querySelector('body').style.overflow = 'visible';
        _this6.$img.style.display = 'none';
      });
    }
  }, {
    key: "clickHandler",
    value: function clickHandler(event) {
      event.preventDefault();
      var $el = event.target.closest('[data-type="gallery-item"]');
      var index = $el.dataset.order;
      this.$img = document.querySelector(".portfolio-item__img-".concat(index));
    ...
    value: function init() {
      ...
      this.$main.addEventListener('click', function (event) {
        return _this7.clickHandler(event);
      });
      this.overlayHandler();
    }
  }, {
    ...
    value: function destroy() {
      ...
      this.$main.removeEventListener('click', function (event) {
        return _this8.clickHandler(event);
      });
    }
  }]);
...
new Gallery('.portfolio-items', '[data-overlay="img"]'); // switch (true) {
//   case this.catet <= 0:
